...
| `docs/base-standards.md` | `docs/` (if present in template; overwrite only if missing unless user asks refresh) |
| `docs/backend-standards.md` | `docs/` (same rule) |
| `docs/frontend-standards.md` | `docs/` (same rule) |
| `docs/data-architecture-discovery.md` | `docs/` (**always copy** — Data Persistency gate) |
| `docs/openspec-tasks-mandatory-steps.md` | `docs/` (if present) |
...
**Data Architecture gate:** New projects MUST receive `docs/data-architecture-discovery.md`. Without it, `/director` and `backend-developer` cannot enforce the Access Pattern Matrix before schemas.

PowerShell reference:

```powershell
$src = $env:OPENSPEC_TEMPLATE_REPO  # e.g. D:\Repositorios\OpenSpecs or OpenSpecs-Template
$dst = Get-Location
Copy-Item -Recurse -Force "$src\ai-specs" "$dst\"
New-Item -ItemType Directory -Force "$dst\openspec\changes\archive","$dst\docs","$dst\tmp" | Out-Null
Copy-Item -Force "$src\openspec\config.yaml" "$dst\openspec\"
Copy-Item -Recurse -Force "$src\openspec\schemas" "$dst\openspec\schemas"
# Standards + Data Architecture Discovery (persistence gate)
foreach ($f in @(
  "base-standards.md",
  "backend-standards.md",
  "frontend-standards.md",
  "data-architecture-discovery.md",
  "openspec-tasks-mandatory-steps.md"
)) {
  if (Test-Path "$src\docs\$f") {
    if ($f -eq "data-architecture-discovery.md" -or -not (Test-Path "$dst\docs\$f")) {
      Copy-Item -Force "$src\docs\$f" "$dst\docs\$f"
    }
  }
}
...
6b. **Always ensure** `docs/data-architecture-discovery.md` exists (copy from template if missing). Link it from `base-standards.md` Specific standards.
