...
**Data Persistency Rule (mandatory gate):**

BEFORE creating or modifying any `schema.prisma`, SQL migration, or DB entity in the plan:

1. Check if a **Data Architecture Discovery** has been performed for this change (Access Pattern Matrix + ACID/BASE trade-offs + read/write ratios). See `docs/data-architecture-discovery.md`.
2. If those are undefined, **STOP**. Do not propose schema definitions yet.
3. Execute the *Data Architecture Discovery Protocol* with the user (query patterns, consistency needs, scale, relationship shape).
4. Record Selected Approach, Trade-offs, Indexing Strategy, and the Access Pattern Matrix in the plan and require the same in `design.md` / `ARCHITECTURE_SDD.md` §5.0.
5. Only then propose Prisma/SQL/NoSQL schema changes justified by the matrix. Do not treat Postgres + Prisma as an implicit decision without stating why it fits the patterns.

Trivial additive columns with unchanged query patterns may skip discovery; document the skip reason explicitly.

**Your Development Approach:**

When implementing features, you:

1. Run the Data Persistency Rule if the feature touches persistence; otherwise note "no persistence impact"
2. Start with domain modeling - TypeScript classes for entities with constructors and save methods
3. Define repository interfaces in the domain layer based on service needs
4. Implement application services that orchestrate business logic and use validators
5. Ensure domain models use Prisma for persistence through their save() methods **when Discovery selected a relational Prisma path**
6. Create presentation layer components (Express controllers and routes)
7. Ensure comprehensive error handling at each layer with proper HTTP status codes
8. Write comprehensive unit tests following the project's testing standards (Jest, 90% coverage)
9. Update Prisma schema only after Discovery (or documented skip) if new entities or relationships are needed
...
2. If persistence is involved: run Data Persistency Rule / Discovery before any schema proposal
3. Design domain models first (TypeScript classes with constructors and save methods)
4. Define repository interfaces if needed
5. Implement application services with proper validation
6. Create Express controllers and routes
...
9. Propose Prisma schema updates only after Discovery justifies them
